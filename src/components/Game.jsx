...
function Game(props) {
  ...
  const theme = THEMES_LIST.find((e) => e.slug === URL_QUERY);
  ...
  function updateCounter(value = undefined) {
    setScores([
      ...scores,
      { question: sessionSet[counter], completed: true, userAnser: value },
    ]);
  ...
  function updateGameStatistics() {
    ...
    const end = new Date(GAME_DATA.ended);
    ...
    for (const el of scores) {
      if (el.userAnser === true) {
        GAME_DATA.right += 1;
        GAME_DATA.points += GAME_PARAMS.POINTS_RIGHT_ANSWER;
      }
      if (el.userAnser === false) {
        GAME_DATA.wrong += 1;
        GAME_DATA.points += GAME_PARAMS.POINTS_WRONG_ANSWER;
      }
      if (el.userAnser === "") {
        GAME_DATA.unanswered += 1;
        GAME_DATA.points += GAME_PARAMS.POINTS_IF_UNANSWERED;
      }
    }
    ...
    if (
      GAME_DATA.totalQuestions * GAME_PARAMS.POINTS_RIGHT_ANSWER * 0.7 <
      GAME_DATA.points
    ) {
      GAME_DATA.message = " well done!";
    } else if (
      GAME_DATA.totalQuestions * GAME_PARAMS.POINTS_RIGHT_ANSWER * 0.4 <
      GAME_DATA.points
    ) {
      GAME_DATA.message = " keep trying!";
    } else if (
      GAME_DATA.totalQuestions * GAME_PARAMS.POINTS_RIGHT_ANSWER * 0.15 <
      GAME_DATA.points
    ) {
      GAME_DATA.message = " don't give up yet!";
    } else {
      GAME_DATA.message = " not your best day.";
    }
  }
